...
* **光照基础知识**
  ...
  2. 漫反射光计算公式：
     ...
     ③ lightDir光源方向：lightDir=lightPos-fragPos;  
  ...
  3. 境面光计算公式：  
      specular = specularStrength _ pow\(max\(dot\(normalize\(reflect\(fragPos-lightPos,N\)\),normalize\(viewPos-fragPos\)\),0.0\),32\) _ lightColor
