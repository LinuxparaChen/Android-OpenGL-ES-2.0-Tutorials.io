...
4. **编写渲染器**
    ...
    注：给着色器属性赋值的方法：
    `attribute`修饰的属性使用：
    `GLES20.glVertexAttrib1f(int indx,float x)`给`attribute`修饰的float类型的标量/数组赋值。
    `GLES20.glVertexAttrib2f(int indx,float x,float y)`给float类型的标量赋值。
    `GLES20.glVertexAttrib3f(...)`给`attribute`修饰的float类型的数组赋值。
    `GLES20.glVertexAttrib4f(...)`给float类型的标量赋值。